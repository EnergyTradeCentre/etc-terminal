    ...
    return (
          <div style={{ height: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#000' }}>
                  <div style={{ border: `1px solid ${theme.border}`, padding: '50px', textAlign: 'center', maxWidth: '400px' }}>
                            ...
                            <h1 style={{ fontFamily: theme.serif, fontSize: '2rem', marginBottom: '10px', color: '#fff' }}>Institutional Access</h1>h1>
                            <p style={{ color: '#666', fontSize: '0.8rem', marginBottom: '30px' }}>ETC Deal Intelligence Terminal v7.3</p>p>
                            ...
                            <button onClick={onVerify} style={{ width: '100%', padding: '12px', background: theme.gb, color: '#000', fontWeight: 'bold', border: 'none', cursor: 'pointer' }}>REQUEST ACCESS</button>button>
                  </div>div>
          </div>div>
        );
}
    ...
    return (
          <div style={{ width: '100%', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', overflowY: 'auto' }}>
                  <div style={{ border: `1px solid ${theme.jp}`, padding: '30px' }}>
                            <h2 style={{ fontFamily: theme.serif, fontSize: '2rem', color: theme.jp }}>Regional Curtailment Engine</h2>h2>
                            <p style={{ fontSize: '0.8rem', color: '#888' }}>Probability of OCCTO intervention by area (2025-2030)</p>p>
                            <div style={{ marginTop: '30px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
                              {Object.entries(data.regions || {}).map(([name, r]) => (
                        <div key={name}>
                                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '5px' }}>
                                                          <span style={{ fontSize: '0.9rem', color: theme.text }}>{name.toUpperCase()}</span>span>
                                                          <span style={{ color: theme.jp }}>{(r.curtailment * 100).toFixed(1)}% RISK</span>span>
                                        </div>div>
                                        <div style={{ height: '2px', background: '#222' }}>
                                                          <div style={{ height: '100%', background: theme.jp, width: `${r.curtailment * 100}%` }}></div>div>
                                        </div>div>
                        </div>div>
                      ))}
                            </div>div>
                  ...
                  <div style={{ border: `1px solid ${theme.border}`, padding: '30px', background: 'rgba(255,49,49,0.02)' }}>
                            <h2 style={{ fontFamily: theme.serif, fontSize: '2rem', color: '#fff' }}>PPA Settlement Forecast</h2>h2>
                            <div style={{ fontSize: '3rem', margin: '20px 0', fontWeight: 'bold', color: '#fff' }}>
                              {data.benchmarks ? `$${data.benchmarks.jkm.toFixed(2)}` : '--'}
                                        <span style={{ fontSize: '1rem', color: '#666' }}> Synthetic JKM</span>span>
                            </div>div>
          ...
          <>
                <div style={{ width: '280px', display: 'flex', flexDirection: 'column', gap: '15px' }}>
                        <div style={{ flex: 1, border: `1px solid ${theme.border}`, padding: '15px' }}>
                                  <h3 style={{ fontSize: '0.7rem', color: '#888', marginBottom: '15px' }}>GENERATION_MIX</h3>h3>
                                  <div style={{ height: '150px', border: `1px solid ${theme.border}`, position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                              ...
                                              <div style={{ position: 'absolute', textAlign: 'center' }}>
                                                            <div style={{ fontSize: '1.2rem', color: '#fff' }}>{market === 'gb' ? '42.1%' : '18.4%'}</div>div>
                                                            <div style={{ fontSize: '0.5rem', color: '#666' }}>RENEWABLE_CAPTURE</div>div>
                                              </div>div>
                                  </div>div>
                                  <div style={{ marginTop: '20px' }}>
                                              <h3 style={{ fontSize: '0.7rem', color: '#888' }}>RISK_SIGNALS</h3>h3>
                          ...
                          <div key={risk} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid #111', fontSize: '0.7rem' }}>
                                          <span style={{ color: theme.text }}>{risk}</span>span>
                                          <span style={{ color: theme.jp }}>CRITICAL</span>span>
                          </div>div>
                        ))}
                                  </div>div>
                        </div>div>
                ...
                <div style={{ flex: 1, border: `1px solid ${theme.border}`, display: 'flex', flexDirection: 'column', background: '#000' }}>
                        <div style={{ flex: 1, padding: '20px', fontSize: '0.85rem', color: '#aaa', overflowY: 'auto' }}>
                                  <div style={{ color: color }}>[SYSTEM_READY] ... Initializing Intelligence Bridge</div>div>
                                  <div style={{ marginTop: '10px' }}>[MARKET_AWARENESS]: Analysis of the {market.toUpperCase()} forward curve indicates spread compression in Q4. Merchant tail risks are currently priced at a 12% discount to historical fundamental value.</div>div>
                ...
                <div style={{ width: '320px', border: `1px solid ${theme.border}`, padding: '15px' }}>
                        <h3 style={{ fontSize: '0.7rem', color: '#888', marginBottom: '15px' }}>BANKABLE_FORWARD_CURVE</h3>h3>
                ...
                <header style={{ height: '60px', borderBottom: `1px solid ${theme.border}`, display: 'flex', alignItems: 'center', padding: '0 25px', justifyContent: 'space-between', background: theme.surface }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '30px' }}>
                                  <div style={{ fontFamily: theme.serif, fontSize: '1.6rem', fontWeight: 'bold', color: '#fff' }}>
                                              ETC <span style={{ fontSize: '0.8rem', fontFamily: theme.mono, color: theme.gb }}>INTEL_TERMINAL</span>span>
            </div>div>
                                  <div style={{ display: 'flex', background: '#000', border: `1px solid ${theme.border}`, borderRadius: '2px', padding: '2px' }}>
                                              <button onClick={() => { setMarket('gb'); setShowJapanEXP(false); }} style={{ padding: '5px 15px', background: market === 'gb' ? theme.gb : 'transparent', color: market === 'gb' ? '#000' : '#fff', border: 'none', cursor: 'pointer', fontSize: '0.7rem', fontWeight: 'bold' }}>GB_MARKET</button>button>
                                              <button onClick={() => setMarket('japan')} style={{ padding: '5px 15px', background: market === 'japan' ? theme.jp : 'transparent', color: '#fff', border: 'none', cursor: 'pointer', fontSize: '0.7rem', fontWeight: 'bold' }}>JAPAN_SYNTH</button>button>
                                  </div>div>
                        </div>div>
                        ...
                        <button onClick={() => setShowJapanEXP(!showJapanEXP)} style={{ background: 'transparent', border: `1px solid ${theme.jp}`, color: theme.jp, padding: '6px 15px', fontSize: '0.7rem', cursor: 'pointer', fontWeight: 'bold' }}>
                          {showJapanEXP ? '[ CLOSE_EXP_ENGINE ]' : '[ RUN_JAPAN_EXP_DEEP_DIVE ]'}
                                  ...
                                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '0.7rem', color: theme.gb }}>
                                              <div style={{ width: '8px', height: '8px', background: theme.gb, borderRadius: '50%', boxShadow: `0 0 10px ${theme.gb}` }}></div>div>
                                              LIVE_FEED_ACTIVE
                                  </div>div>
                        </div>div>
                </header>header>
